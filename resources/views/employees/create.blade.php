...
<div class="block mx-auto my-40 p-8 bg-white w-1/3 border border-gray-200 rounded-lg shadow-lg">
        ...
        <form action="{{route('employees.store')}}" class="mt-8" method="POST">
            @csrf
            ...
            placeholder-gray-900 p-2 my-2 focus:bg-white" value="{{old('FirstName')}}">

            <input type="text" name="LastName" placeholder="Apellidos"
            id="LastName" class="border border-gray-200 rounded-md bg-gray-200 w-full
            placeholder-gray-900 p-2 my-2 focus:bg-white" value="{{old('LastName')}}">
            ...
            placeholder-gray-900 p-2 my-2 focus:bg-white">
                ...
                @foreach($data as $puesto)
                    <option value="{{$puesto['id']}}" {{ old('Stall_id') == $puesto['id'] ? 'selected' : ''}}>
                        {{$puesto['Name']}}
                    </option>
                @endforeach
            </select>
            ...
            placeholder-gray-900 p-2 my-2 focus:bg-white" value="{{old('Phone')}}">
